...
class View
{
	...
	public static function display ($path)
	{
		...
		// 获取模板文件名，防止全路径
		$templete = \last(\explode('/', $path));
		
		if ( !file_exists($handle) ) {
			throw new ErrorException("模板文件{$templete}.html不存在", 301);
		}
		$tempConReplace = $templeteContent = htmlspecialchars(file_get_contents($handle));
		...
		$templeteContent = htmlspecialchars_decode($tempConReplace);
		...
		file_put_contents($cache_fileName, $templeteContent);
		
		exit($templeteContent);
	}
